...
Results can also include a consultant from CollabFinder's own trusted-expert directory: name, credentials, rate, and a booking link. Experts are recruited and credential-verified by the platform; only verified entries are shown. Each expert sets their own commission rate at signup (10% minimum), and among experts matching a query, the highest commission places first — relevance gates entry, commission orders placement. Booking and payment run through CollabFinder's hosted booking page; the platform withholds the commission and pays out the expert (Stripe capture is the launch build; booking flow and attribution logging are implemented).
